...
class DmsSecurityMixin(models.AbstractModel):
    ...
    def _get_domain_by_inheritance(self, operation):
        ...
        related_data = self.sudo().search_read(
            domain=inherited_access_domain + [("res_model", "!=", False)],
            fields=["res_model", "res_id"],
        )
        related_groups_dict = defaultdict(list)
        for item in related_data:
            related_groups_dict[item["res_model"]].append(item["res_id"])

        for res_model, res_ids in related_groups_dict.items():
            try:
