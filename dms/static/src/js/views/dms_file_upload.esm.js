...
export function createFileUploadExtension() {
    return {
        ...
        async onUpload(attachments) {
            ...
            const search_domains = [this.props.domain];
            if (this.model && this.model.root && this.model.root.domain) {
                search_domains.push(this.model.root.domain);
            }

            for (const domain of search_domains) {
                if (domain && Array.isArray(domain)) {
                    for (const domain_item of domain) {
                        if (Array.isArray(domain_item) && domain_item.length === 3) {
                            if (
